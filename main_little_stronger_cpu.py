...
class App(tk.Tk):
    def __init__(self):
        ...
        self.bind("<Button-1>", self.player_input)  # クリック操作を受け取る
        ...
        self.current_movable_tag = None
        self.opponent_movable = None
        self.player_color = None

        # 処理待機のフラグを設定
        self.wait_for_cpu = True
        self.game_over = False

        # 初期配置
        self.refresh_board()

        self.wait_for_cpu = False
    ...
    def refresh_board(self):
        ...
        # ランダムでプレイヤー手番を設定
        self.player_color = random.choice(COLOR)
        ...
        self.set_movable_highlight()

    def player_input(self, event):
        if self.wait_for_cpu or self.game_over:
            return
        ...
        if self.current_movable[x_index][y_index] != 0:
            ...
            passed = self.change_turn()
            if not passed:
                self.after(1000, self.cpu_input)
            else:
                self.set_movable_highlight()
    ...
    def cpu_input(self):
        ...
        # ランダムで石を置ける位置から1か所を選択
        if self.current_turn < 30:
            dic = sorted(self.current_movable_tag.items(), key=lambda x: x[1], reverse=True)
        else:
            dic = sorted(self.current_movable_tag.items(), key=lambda x: x[1])

        random_choice_tag = dic.pop()[0]

        self.put_piece(random_choice_tag, self.current_color)
        self.reverse_piece(random_choice_tag)
        self.change_turn()
        self.set_movable_highlight()
            ...
            while self.raw_board[x_tmp][y_tmp] == - color:
                ...
                if count != 0 and self.raw_board[x_tmp][y_tmp] == color:
                    if color == self.current_color:
                        ...
                        # cpu_inputのため、石を置ける位置をtagで保管
                        self.current_movable_tag[tag] = count

                    else:  # 相手が石を置ける場所の判定を行う場合はハイライトは付けない
                        self.opponent_movable[x_index][y_index] |= movable_dir_bit

    def set_movable_highlight(self):
        for tag in self.current_movable_tag:
            x_point, y_point = self.tag_to_point(tag)
            self.canvas.create_rectangle(x_point, y_point, x_point + 40, y_point + 40,
                                         fill="yellow", tags="movable_highlight")
